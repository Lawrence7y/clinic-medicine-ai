...
import com.ruoyi.framework.shiro.web.filter.authc.ApiTokenSecurityFilter;
import com.ruoyi.framework.shiro.web.filter.authc.ApiUserFilter;
...
public class ShiroConfig
{
     ...
     */
    @Value("${shiro.token.replayProtectEnabled:true}")
    private boolean replayProtectEnabled;

    /**
     * API Token 是否强制客户端绑定标识
     */
    @Value("${shiro.token.requireClientKey:true}")
    private boolean requireClientKey;

    /**
     * API Token 请求时间窗口（秒）
     */
    @Value("${shiro.token.requestWindowSeconds:300}")
    private int requestWindowSeconds;

    /**
     * API Token 防重放缓存上限
     */
    @Value("${shiro.token.maxNonceCacheSize:512}")
    private int maxNonceCacheSize;
    ...
    public ShiroFilterFactoryBean shiroFilterFactoryBean(SecurityManager securityManager)
    {
        ...
        // 小程序/API 登录与注册：不使用验证码，实现与后端账号互通
        filterChainDefinitionMap.put("/api/auth/captcha", "anon");
        filterChainDefinitionMap.put("/api/auth/login", "anon");
        filterChainDefinitionMap.put("/api/auth/register", "anon");
        // 诊所配置（诊所名称、电话等公开信息，登录页展示用）
        filterChainDefinitionMap.put("/api/clinic/config/get", "anon");
        filterChainDefinitionMap.put("/api/clinic/config/version", "anon");
        filterChainDefinitionMap.put("/api/**",
                "apiUser,apiTokenSecurity,kickout,onlineSession,syncOnlineSession,csrfValidateFilter");
        // 系统权限列表
        // filterChainDefinitionMap.putAll(SpringUtils.getBean(IMenuService.class).selectPermsAll());

        Map<String, Filter> filters = new LinkedHashMap<String, Filter>();
        filters.put("apiUser", new ApiUserFilter());
        filters.put("apiTokenSecurity", apiTokenSecurityFilter());
     ...
     */
    public ApiTokenSecurityFilter apiTokenSecurityFilter()
    {
        ApiTokenSecurityFilter filter = new ApiTokenSecurityFilter();
        filter.setReplayProtectEnabled(replayProtectEnabled);
        filter.setRequireClientKey(requireClientKey);
        filter.setRequestWindowSeconds(requestWindowSeconds);
        filter.setMaxNonceCacheSize(maxNonceCacheSize);
        return filter;
    }
